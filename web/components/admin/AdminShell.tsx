...
const navItems: Array<{ key: AdminNavKey; label: string; href: string; icon: React.ComponentType<{ className?: string }> }> = [
    { key: "overview", label: "Overview", href: "/admin", icon: ChartColumnIncreasing },
    { key: "lost-found", label: "Lost & Found", href: "/admin/lost-found", icon: Briefcase },
        ...
        <div className="min-h-screen bg-[hsl(var(--surface))]">
            <aside className="hidden lg:fixed lg:inset-y-0 lg:flex lg:w-72 lg:flex-col bg-primary text-primary-foreground">
                <div className="px-8 py-4">
                    <Image src="/logo/logo.png" alt="CampusLink Logo" width={180} height={180} className="mr-2 inline" />
                </div>
                ...
                <div className="mt-auto px-6 pb-6">
                    ...
                    <div className="mt-6 rounded-xl border border-white/10 bg-white/5 px-4 py-3">
                        ...
                        <p className="text-xs text-primary-foreground/70">Senior Moderator</p>
                    </div>
                </div>
            </aside>
                ...
                <header className="sticky top-0 z-40 border-b border-border/60 bg-card/95 backdrop-blur">
                    <div className="flex h-16 items-center justify-between gap-3 px-4 sm:px-6 lg:px-8">
                        <div>
                            ...
                            <h1 className="text-sm font-bold text-primary sm:text-base">{title}</h1>
                        </div>
                        <div className="hidden items-center gap-2 md:flex">
                            <div className="relative">
                                <Search className="pointer-events-none absolute left-3 top-1/2 h-4 w-4 -translate-y-1/2 text-muted-foreground" />
                                <Input className="h-10 w-64 rounded-full bg-muted/60 pl-9" placeholder="Global university search..." />
                            </div>
